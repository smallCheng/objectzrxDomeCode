...
//-----------------------------------------------------------------------------
IMPLEMENT_DYNAMIC (CMyPaletteSet, CZdUiPaletteSet)

BEGIN_MESSAGE_MAP(CMyPaletteSet, CZdUiPaletteSet)
END_MESSAGE_MAP()
...
void CMyPaletteSet::GetMinimumSize (CSize &size) {
	...
	//CZdUiPaletteSet::GetMinimumSize (size) ;
...
void CMyPaletteSet::GetMaximumSize (CSize &size) {
	...
	//CZdUiPaletteSet::GetMaximumSize (size) ;

	//面板的最大允许尺寸（在AutoCAD中视乎无效）
	size.cx=400;
	size.cy=400;
}
