...
CModelessDialog *pDialog=NULL;	//非模态对话框
CMyDockControlBar *g_pDlgBar=NULL;	//可停靠窗体
CMyPaletteSet *g_pPaletteSet=NULL;	//选项板集合
CMyPalette *g_pPalette1=NULL;	//选择板
CZdUiPalette *g_pPalette2=NULL;

CMyEditorReactor *g_enitorReactor=NULL;	//编辑反应器
CMyDocReactor	*g_docReactor=NULL;	//文档反应器
CMyDbReactor *g_dbReactor=NULL;	//数据库反应器

CAcExtensionModule PreviewControlDLL;

#pragma endregion
///////////////////////////////////////
/**********释放窗口资源***************/
#pragma region	释放窗口资源
...
BOOL CloseDialog()
{
	...
	BOOL bRrt=pDialog->DestroyWindow();
...
BOOL CloseBar()
{
	...
	BOOL bRrt=g_pDlgBar->DestroyWindow();
	if(bRrt)
	{
		g_pDlgBar=NULL;
	}
...
public:
	...
	virtual AcRx::AppRetCode On_kInitAppMsg (void *pkt) {
		...
		//添加编辑反应器
		//g_enitorReactor=new CMyEditorReactor();
		//acedEditor->addReactor(g_enitorReactor);
	...
	virtual AcRx::AppRetCode On_kUnloadAppMsg (void *pkt) {
		...
		//删除编辑反应器
		//acedEditor->removeReactor(g_enitorReactor);
	...
	static void zwzw_AddSpire()
	{
		...
		double angle = 8 * atan(1.0) / segment;	// 两点之间的旋转角度
		...
		{
			AcGePoint3d vertex;
			vertex[X] = radius * cos(8 * i * atan(1.0)/ segment);
			vertex[Y] = radius * sin(8 * i * atan(1.0) / segment);
	...
	static void zwzw_AddRevolvEnt()
	{
		...
		{
			AcDb3dSolid *pSolid = new AcDb3dSolid();
			pSolid->revolve(pRegion, AcGePoint3d::kOrigin, AcGeVector3d(0, 1, 0), 8 * atan(1.0));
			CDwgDatabaseUtil::PostToModelSpace(pSolid);

			pRegion->close();
		}
	}
	static void zwzw_Boolean()
	{
		// 创建两个长方体
	...
	static void zwzw_ShowTabDlg()
	{
		CAcModuleResourceOverride myResoure;
		COptionSheet dlg;
		dlg.DoModal(); 
	}
	static void zwzw_ShowBar()
	{
		//显示可停靠窗体
		CAcModuleResourceOverride resOverride;
		if(g_pDlgBar==NULL)
		{
			...
			g_pDlgBar->SetWindowTextW(_T("MyControBar"));
			g_pDlgBar->EnableDocking(CBRS_ALIGN_ANY);
		}
		acedGetAcadFrame()->FloatControlBar(g_pDlgBar,CPoint(300,300),CBRS_ALIGN_ANY);
		acedGetAcadFrame()->ShowControlBar(g_pDlgBar,TRUE,TRUE);
	}
	static void zwzw_AddPalette()
	{
		if(g_pPaletteSet==NULL)
		{
			//创建paletteset
			g_pPaletteSet=new CMyPaletteSet;
			CRect rect(0,0,400,400);	//初始化大小
			g_pPaletteSet->Create(_T("自定义面板"),WS_OVERLAPPED|WS_DLGFRAME,rect,acedGetAcadFrame(),PSS_EDIT_NAME|PSS_PROPERTIES_MENU|PSS_AUTO_ROLLUP|PSS_CLOSE_BUTTON);

			//创建Palette实例
			g_pPalette1=new CMyPalette();	//自定义的选项版
			g_pPalette2=new CZdUiPalette();
			//系统默认的选项板，只是为了能显示标签名

			//选项板1
			g_pPalette1->Create(WS_CHILD|WS_VISIBLE,_T("选 项 板 1"),g_pPaletteSet,PSS_EDIT_NAME);
			//添加到选项板集合
			g_pPaletteSet->AddPalette(g_pPalette1);

			//选项板1
			g_pPalette2->Create(WS_CHILD|WS_VISIBLE,_T("选 项 板 2"),g_pPaletteSet,PSS_EDIT_NAME);
			//添加到选项板集合
	...
	static void zwzw_PreviewDlg()
	{
		CAcModuleResourceOverride myResource;
		CPreviewDlg dlg;
		dlg.DoModal();
	}
	static void zwzw_ShowGridDlg()
	{
		CAcModuleResourceOverride myResource;
		CGridDlg dlg;
		dlg.DoModal();
	}
	static void zwzw_SaveFile()
	{
		// 获取直线、圆形集合
		AcDbObjectIdArray allEntIds = CDwgDatabaseUtil::GetAllEntityIds();
		...
		// 写二进制文件
		CString fileName = CAppDirectoryUtil::GetCurrentDirctory(_hdllInstance) + TEXT("\\二进制文件.bnf");
	...
	static void zwzw_ImportFromExcel()
	{
		//// 1.提示用户选择Excel文件
	...
	static void zwzw_SetLinkEnts()
	{
		// 提示用户选择两个圆
		AcDbObjectIdArray circleIds;
		AcRxClass *pDesc=AcDbCircle::desc();
		if (CSelectUtil::PromptSelectEnts(TEXT("\n选择两个圆:"), pDesc, circleIds))
		{
			...
			Acad::ErrorStatus es = acdbHostApplicationServices()->workingDatabase()->getNamedObjectsDictionary(pNamedDict, AcDb::kForWrite);
...
/*第十章：三维*/
//创建长方体
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_AddBox, AddBox, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//创建圆锥体
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_AddCylinder, AddCylinder, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//创建三维弹簧
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_AddSpire, AddSpire, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//旋转面域生成一个实体
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_AddRevolvEnt, AddRevolvEnt, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//对两个长方体进行布尔运算
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_Boolean, Boolean, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)


/*第十一章：在ObjectArx中使用MFC*/
//创建模块对话框
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_MfcModal, MfcModal, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//Arx模态对话框，获取点坐标与角度
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_ArxModal, ArxModal, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//创建非模态对话框，获取两点坐标创建直线
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_ModelessDlg, ModelessDlg, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//显示Tab对话框
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_ShowTabDlg, ShowTabDlg, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//加载可停靠窗口
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_ShowBar, ShowBar, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//加载工具面板
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_AddPalette, AddPalette, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//显示工具栏
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_DrawTunel, DrawTunel, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//图形预览
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_PreviewDlg, PreviewDlg, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//MFC_GridCtrl
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_ShowGridDlg, ShowGridDlg, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)


/*第十二章：外部文件和数据库*/
//读取直线、圆参数放入二进制文件中
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_SaveFile, SaveFile, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//从二进制文件读取直线、圆参数，在当前模型空间绘制
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_LoadFile, LoadFile, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//将实体信息导入的Excel
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_ExportExcel, ExportExcel, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
//到处Excel信息
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_ImportFromExcel, ImportFromExcel, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)

/*第十三章：反应器*/
//提示用户选择两个圆，将这两个圆互相设置为关联实体
ACED_ARXCOMMAND_ENTRY_AUTO(CZrxTemplate1App, zw, zw_SetLinkEnts, SetLinkEnts, ACRX_CMD_MODAL | ACRX_CMD_USEPICKSET, NULL)
